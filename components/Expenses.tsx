...
import { addTransaction, fetchTodayExpenses, deleteTransaction, getTodayString, fetchCategories } from '../services/supabase';
...
export const Expenses: React.FC<ExpensesProps> = ({ onBack, onNavigate, currentUser }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const sum = expData.reduce((acc, curr) => acc + (curr.amount || 0), 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh list
      const expData = await fetchTodayExpenses();
      setHistory(expData);
      const sum = expData.reduce((acc, curr) => acc + (curr.amount || 0), 0);
      setTotalExpenses(sum);
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="space-y-6 pb-24">
        ...
        {/* Top Summary Card */}
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl p-3 text-white shadow-md shadow-orange-200/50 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-1.5 text-orange-100 text-[10px] uppercase font-bold tracking-wider mb-0.5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:border-orange-500 focus:bg-white outline-none transition-all text-slate-800 appearance-none cursor-pointer text-sm font-medium"
                    required
                  >
              ...
              <div className="space-y-1.5">
                 ...
                 <textarea
                   ...
                   className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:border-orange-500 focus:bg-white outline-none transition-all text-slate-800 placeholder:text-slate-400 resize-none text-sm"
                 />
              </div>
              ...
              <div className="pt-2">
                <Button 
                  ...
                  isLoading={loading} 
                  variant="danger"
                  className="bg-orange-600 hover:bg-orange-700 shadow-orange-200 py-3 text-sm"
                >
                  Save Expense
                </Button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto min-h-[300px] md:min-h-0 bg-slate-50/50 rounded-3xl border border-slate-100 p-2 md:p-3">
              {loadingHistory ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8">
                  ...
                  <p className="text-xs font-medium">No expenses today</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {history.map((item) => (
                    <div key={item.id} className="group bg-white p-3 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-orange-200 transition-all flex items-center gap-3">
